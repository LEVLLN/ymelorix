//! Сценарий «загрузить по ссылке»: трек, альбом или плейлист.
...
///
/// Одиночный трек — тот же список, только из одного элемента: у него нет ни
/// своей команды, ни своего пути в коде, поэтому повторы, обработку `429` и
/// пропуск по `--update` он получает ровно те же, что и альбом.
///
/// Авторизацию отдельным запросом здесь не проверяем, в отличие от избранного:
/// там `uid` нужен самой ручке лайков, а всё остальное адресуется ссылкой.
...
pub(crate) async fn run(
    ...
    let tracks = match &source {
        Source::Track(id) => vec![library::track_by_id(client, id).await?],
